...
def readImRePhase(folderPathList, fileNameIm, fileNameRe, scale):
    """
...
def plotSimulatedImReCurves_Linear(filePath, saveFigPath, scale, maxPos=107, maxTick=12, tickStep=2):
    '''
...
def readImRe(folderPath, fileNameIm, fileNameRe, scale):
    ...
    with open(filePathIm) as f:
        ...
        lines = data.split("\n")
        for line in lines:
            im_img_array.append(np.array(line.split("\t")).astype(float) * scale)
    re_img_array = []
    with open(filePathRe) as f:
        data = f.read()[:-1]
        lines = data.split("\n")
        for line in lines:
            re_img_array.append(np.array(line.split("\t")).astype(float) * scale)
    return np.asarray(im_img_array), np.asarray(re_img_array)


def mim2Conductivity(folderPath, fileNameIm, fileNameRe, cond_array, im_sim, re_sim):
    im_img_array, re_img_array = readImRe(folderPath, fileNameIm, fileNameRe)
